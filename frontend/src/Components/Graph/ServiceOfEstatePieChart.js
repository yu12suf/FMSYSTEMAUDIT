...
// Custom Legend Renderer for 3 columns
    ...
    const fetchChartData = async () => {
      try {
        ...
        const response = await axiosInstance.get(
          "http://localhost:8000/api/statistics/service-of-estate"
        );
        ...
        console.error("Error fetching Service of Estate chart data:", error);
        setError(
          "Failed to load Service of Estate data. Please check the backend."
  ...
  if (chartData.length === 0 && !error) {
    return (
      <div style={{ textAlign: "center", padding: "20px" }}>
        No data available for Service of Estate chart.
      </div>
    );
  }
